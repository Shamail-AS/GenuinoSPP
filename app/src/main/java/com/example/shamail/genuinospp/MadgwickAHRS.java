...
public class MadgwickAHRS {
    ...
    public void update(float gx, float gy, float gz, float ax, float ay,
        ...
        norm = 1f / (float) Math.sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4); // normalise
        // quaternion
        quaternion[0] = q1 * norm; //w
        quaternion[1] = q2 * norm; //x
        quaternion[2] = q3 * norm; //y
        quaternion[3] = q4 * norm; //z
    }

    public float getRoll()
    {
        return (float)Math.atan2(2*quaternion[0]*quaternion[2] - 2*quaternion[1]*quaternion[3],
                1- 2*quaternion[2]*quaternion[2] - 2*quaternion[3]*quaternion[3]);
    }

    public float getPitch()
    {
        return (float)Math.atan2(2*quaternion[0]*quaternion[1] - 2*quaternion[2]*quaternion[3],
                1- 2*quaternion[1]*quaternion[1] - 2*quaternion[3]*quaternion[3]);
    }

    public float getYaw()
    {
        return (float)Math.asin(2*quaternion[1]*quaternion[2] + 2*quaternion[3]*quaternion[0]);
    }
