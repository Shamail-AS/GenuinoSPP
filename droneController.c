...
float pid_i_gain_yaw=0;float pid_i_mem_yaw=0;
...
void setup() {
  ...
  */
  ...
  CurieIMU.setGyroRange(250); //degrees per sec
...
void loop() {
  ...
  if(micros() >= timestamp)
  {
   ...
   // RunMotors();
    timestamp = micros() + 4000; //run program at 200 hertz
...
void getPosition()
{
  ...
  //integrate
  roll += (rate_roll/200) ; // rate * time period OR (rate / freq)
  pitch += (rate_pitch/200) ;
  yaw += (rate_yaw/200);

  float forceMagApprox = abs(ax)+abs(ay)+abs(az);
  if(forceMagApprox > 8192 && forceMagApprox < 32768)
  {
    float acc_pitch = atan2((float)ay,(float)az); // radians around x axis
    pitch = pitch * 0.90 + acc_pitch * 0.10;
    float acc_roll = atan2((float)ax,(float)az); // around y axis
...
void RunMotors()
{
  ESC9.writeMicroseconds(esc1);
  ESC6.writeMicroseconds(esc2);
  ESC5.writeMicroseconds(esc3);
  ESC3.writeMicroseconds(esc4);
}
...
void PID(float set_point_roll, float set_point_pitch, float set_point_yaw)
{
  ...
  */
  ...
  esc1=throttle-output_pitch-output_roll+output_yaw;//-pitchTrim-rollTrim+outputYaw; // ESC9 White wire - Front right - pin9
  esc2=throttle+output_pitch+output_roll+output_yaw+round(1.5*pitchTrim);//+pitchTrim+rollTrim+outputYaw; // ESC6 Orange wire - Rear left - pin6

  esc3=throttle+output_pitch-output_roll-output_yaw+round(1.5*pitchTrim);//+pitchTrim-rollTrim-outputYaw; // ESC5 Red wire - Rear right
  esc4=throttle-output_pitch+output_roll-output_yaw;//-pitchTrim+rollTrim-outputYaw; // ESC3 Grey wire - Front left - pin3
